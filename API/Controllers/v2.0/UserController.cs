...
namespace API.Controllers.v2_0
{
    ...
    public class UserController : Controller
    {
        ...
        private readonly INewsletterV2 _newsletter;
        ...
        private IControllerSubModulesV2 _subModule;
        private readonly IValidateV2 _validate;
        private IMyMFA _MyMFA;
        ...
        public UserController(AppDbContext context, UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, ILogger<UserController> logger,
            INewsletterV2 newsletter, IHttpContextAccessor httpContextAccessor, IPasswordHasher<AppUser> passwordHasher, IPasswordValidator<AppUser> passwordValidator,
            IUserValidator<AppUser> userValidator, IOptions<EmailConfigV2> emailConfig, IEmailServiceV2 email, IControllerSubModulesV2 subModule, IValidateV2 validate, 
            IMyMFA MyMFA, IOptions<MyMFASettings> MyMFASettings)
        {
            _context = context;
        ...
        {
            Debug.WriteLine("V2.0 HELLO WORKS!!!!");

            using (LogContext.PushProperty("ApiVersion", version))
            {
                _logger.LogInformation("ApiVersion: {ApiVersion} - Hello Controller V2.0!", "v" + version);
            }
            ServiceResponse<bool> response = new ServiceResponse<bool>();
            response.Success = true;
            response.Message = "Hello! V2.0";
            return Ok(response);
        }
        ...
        {
            Debug.WriteLine("V2.0 SECURED BASIC HELLO WORKS!!!!");

            using (_logger.BeginScope(new Dictionary<string, object>() { { "ApiVersion", version } }))
            {
                _logger.LogInformation($"ApiVersion: {version} - Secured Basic Hello Controller V2.0!");
            }

            ServiceResponse<bool> response = new ServiceResponse<bool>();
            response.Success = true;
            response.Message = "Secured Basic Hello Works! V2.0";
            return Ok(response);
        }
                ...
                if (passValidation == 0)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if (await _userManager.IsLockedOutAsync(user) == false)
                        {
                            ...
                            {
                                // Is Multi Factor Auth Enabled on their account?
                                if (user.EnableMFA == true)
                                {
                                    // First login will just be username & password, no code supplied.
                                    // We tell the frontend MFA is enabled and to ask for the MFA code.
                                    // Send the user an MFA code by Email.
                                    if (request.MfaCode == "empty" || request.MfaCode == null)
                                    {
                                        // MFA Code Pruning
                                        bool pruning = await _MyMFA.CodePruning(user);

                                        // Create and Email the MFA Code
                                        bool codeSent = await _MyMFA.SendCode(user);

                                        // User has now provided correct username and password.
                                        // Generate Mfa JWT Token so the user can acces the MfaLogin endpoint without resubmitting a password.
                                        // Passing a password between pages in the front end is too insecure, using local storage, just use a temp JWT token.
                                        var mfaToken = await _subModule.GenerateMfaToken(user);

                                        // Signal to the front end that this account has MFA enabled.
                                        LoginResponseDto mfaLoginReply = new LoginResponseDto();
                                        mfaLoginReply.Username = user.UserName;
                                        mfaLoginReply.MFA_Enabled = true;
                                        mfaLoginReply.MfaToken = mfaToken.MfaToken;
                                        mfaLoginReply.MfaTokenExpiry = mfaToken.DateTokenExpires;

                                        response.Data = mfaLoginReply;
                                        response.Success = false;
                                        response.Message = "MFA is enabled for the account.";

                                        // Http 202 status indicates that the request has been accepted but the processing is not yet complete. 
                                        // We don't want to give the user a JWT code until they have been fully authenticated.
                                ...
                                loginReply.MFA_Enabled = user.EnableMFA;
        ...
        // MFA Login
        // Multi-factor Auth Code supplied
        [AllowAnonymous]
        [HttpPost]
        [Route("MfaLogin")]
        public async Task<ActionResult<ServiceResponse<LoginResponseDto>>> MfaLogin(MfaTokenDto request, ApiVersion version)
        {
            try
            {
                var response = new ServiceResponse<LoginResponseDto>();

                int passValidation = _validate.jwtTokenValidation(request.MfaToken) + _validate.alphabetValidation(request.UserName);
                ...
                if (passValidation == 0)
                {
                    ...
                    var isValid = _subModule.ValidateMfaToken(request.MfaToken);
                    ...
                    if (isValid)
                    {
                        ...
                        if (user != null)
                        {
                            ...
                            string? tokenExists = await _context.MfaTokenTable.Where(t => t.MfaToken == request.MfaToken).Select(t => t.MfaToken).FirstOrDefaultAsync();
                            ...
                            if (tokenExists != null)
                            {
                                ...
                                var tokenIsCurrent = await _subModule.MfaTokenIsCurrent(tokenExists);

                                if (tokenIsCurrent == true)
                                {
                                    // Check that refresh token is assigned to the supplied user.
                                    var tokenIsAssignedToUser = await _subModule.MfaTokenIsAssignedToUser(tokenExists, user.Id);
                                    ...
                                    if (tokenIsAssignedToUser == true)
                                    {
                                        ...
                                        // Also removes the currently used code from the database.
                                        bool codeIsConfirmed = await _MyMFA.ConfirmCode(user, request.MfaCode);

                                        if (codeIsConfirmed == false)
                                        {
                                            // Failed MFA
                                            response.Success = false;
                                            response.Message = "MFA code is not correct.";
                                            return Unauthorized(response);
                                        }
                                        else
                                        {
                                            ...
                                            var mfaTokenPruning = await _subModule.MfaTokenPruning(user);
                                    ...
                                    {
                                        // Fail
                                        // MFA token is not assigned to the supplied user!
                                        _logger.LogError("ApiVersion: {ApiVersion} - MFA Token doesn't match the user in our database. UserController.MfaLogin - USER: {0}", version, request.UserName);

                                        response.Success = false;
                                        response.Message = "MFA Token Doesn't Match";
                            ...
                            else
                            {
                                ...
                                _logger.LogError("ApiVersion: {ApiVersion} - MFA Token Doesn't Match. UserController.MfaLogin - USER: {0}", version, request.UserName);
                                _logger.LogError("ApiVersion: {ApiVersion} - UserController.MfaLogin - tokenIsCurrent returned false. MFA Token Supplied: {0}", version, request.MfaToken);


                                response.Success = false;
                                response.Message = "Mfa Token Doesn't Exist.";
                        ...
                        {
                            // User Not Found
                            _logger.LogError("ApiVersion: {ApiVersion} - User Not Found. UserController.MfaLogin - USER: {0}", version, request.UserName);
                    ...
                    {
                        // Invalid Token.
                        _logger.LogError("ApiVersion: {ApiVersion} - Invalid MFA Token. UserController.MfaLogin - USER: {0}", version, request.UserName);

                        response.Success = false;
                        response.Message = "Invalid MFA Token.";
                ...
                _logger.LogInformation("ApiVersion: {ApiVersion} - Validation Failed in UserController.MfaLogin - USER: {0}", version, request.UserName);

                return NotFound(response);
            }
            catch (Exception ex)
            {
                _logger.LogError("ApiVersion: {ApiVersion} - Exception in UserController.MfaLogin - USER: {0} - ERROR: {1}", version, request.UserName, ex.Message);
                _logger.LogError("ApiVersion: {ApiVersion} - Exception in UserController.MfaLogin - MFA Token Supplied: {0}", version, request.MfaToken);
            ...
            try
            {
                ...
                if (passValidation == 0)
                {
                    ...
                    if (user != null && userCheck == true)
                    {
                        ...
                        userObject.EnableMFA = user.EnableMFA;
        ...
        [HttpPost]
        [Route("ToggleMFA")]
        public async Task<ActionResult<ServiceResponse<bool>>> ToggleMFA(ToggleMFADto request, ApiVersion version)
        {
            try
            {
                var response = new ServiceResponse<bool>();
                ...
                if (passValidation == 0)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        user.EnableMFA = request.MfaSwitch;

                        // Update the user in the database, save UnconfirmedEmail.
                        var updateMFA = await _userManager.UpdateAsync(user);

                        if (updateMFA.Succeeded)
                        {
                            // Success!
                            response.Data = user.EnableMFA;                     // Return the actual value of the MFA state as the bool "Data"
                            response.Success = true;
                            response.Message = "MFA has been changed.";
                            return Ok(response);
                        ...
                        else
                        {
                            ...
                            _logger.LogInformation("ApiVersion: {ApiVersion} - Email Update failed. _userManager.UpdateAsync() failed. in UserController.ToggleMFA - USER: {1} Error: {2}", version, request.UserName, updateMFA);
                    ...
                    _logger.LogInformation("ApiVersion: {ApiVersion} - User Doesn't Exist in UserController.ToggleMFA - USER: {0}", version, request.UserName);
                ...
                _logger.LogInformation("ApiVersion: {ApiVersion} - Validation Failed in UserController.ToggleMFA - USER: {0}", version, request.UserName);
            ...
            {
                _logger.LogError("ApiVersion: {ApiVersion} - Exception in UserController.ToggleMFA - {0}", version, ex.Message);
